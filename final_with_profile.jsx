...
import { ChevronDown, Sun, Moon, Briefcase, GraduationCap, Code, Award, User, MessageSquare } from "lucide-react";
    ...
    <div className={`${darkMode ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-900"} min-h-screen`}>
      ...
      {/* Profile Section */}
        <section className="flex flex-col items-center text-center mt-20 py-12">
          <motion.div
            className="w-52 h-52 md:w-64 md:h-64 rounded-full overflow-hidden border-4 border-yellow-500 shadow-xl transition-transform transform hover:scale-110"
          >
            <img src="https://akm-img-a-in.tosshub.com/indiatoday/images/story/202104/Mahesh_Babu_self_isolation_0_1200x768.jpeg?size=690:388" alt="Profile Picture" className="w-full h-full object-cover" />
          </motion.div>
      ...
      <main className="mt-0 container mx-auto p-10 grid grid-cols-1 gap-12">
        {sections.map((section, index) => (
          <motion.section id={section.id} key={index} initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: index * 0.2 }} whileHover={{ scale: 1.05 }} className={`${darkMode ? 'p-10 bg-gray-800 text-white' : 'p-10 bg-white text-black'} rounded-lg shadow-xl flex items-center space-x-4 transition-all hover:bg-gray-200` }>
        ...
        {/* Contact Me Section */}
        <motion.section id="ContactMe" initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }} whileHover={{ scale: 1.02 }} className={`${darkMode ? 'p-10 bg-gray-800 text-white hover:bg-gray-700' : 'p-10 bg-white text-black hover:bg-gray-200'} rounded-lg shadow-xl transition-all` }>
          ...
          <form className="flex flex-col gap-4 mt-4" onSubmit={handleSubmit}>
            <input type="text" name="name" placeholder="Your Name" value={formData.name} onChange={handleInputChange} className={`${darkMode ? 'p-3 rounded bg-gray-700 text-white' : 'p-3 rounded bg-gray-200 text-black'}`} required />
            <input type="email" name="email" placeholder="Your Email" value={formData.email} onChange={handleInputChange} className={`${darkMode ? 'p-3 rounded bg-gray-700 text-white' : 'p-3 rounded bg-gray-200 text-black'}`} required />
            <textarea name="message" placeholder="Your Message" value={formData.message} onChange={handleInputChange} className={`${darkMode ? 'p-3 rounded bg-gray-700 text-white' : 'p-3 rounded bg-gray-200 text-black'}`} required></textarea>
